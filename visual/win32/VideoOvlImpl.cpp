...
//---------------------------------------------------------------------------
// Select the optional krmovie.dll backend when it is present.  The built-in
// implementation remains the fallback for distributions without the DLL.
class tTVPVideoModule
{
	...
	tGetVideoOverlayObject procGetMixingVideoOverlayObject;
...
{
	// Start with the statically linked implementation.  This is also used
	// when krmovie.dll is not packaged with the application.
	procGetVideoOverlayObject = ::GetVideoOverlayObject;
	procGetVideoLayerObject = ::GetVideoLayerObject;
	procGetMixingVideoOverlayObject = ::GetMixingVideoOverlayObject;
	procGetMFVideoOverlayObject = ::GetMFVideoOverlayObject;

	Holder = new tTVPPluginHolder(TJS_W("krmovie.dll"));
	if(Holder->GetLocalName().IsEmpty())
	{
		delete Holder;
		Holder = NULL;
		return;
	...
	try
	{
		...
		procGetVideoLayerObject = (tGetVideoOverlayObject)GetProcAddress(Handle, "GetVideoLayerObject");
		procGetMixingVideoOverlayObject = (tGetVideoOverlayObject)GetProcAddress(Handle, "GetMixingVideoOverlayObject");
		procGetMFVideoOverlayObject = (tGetVideoOverlayObject)GetProcAddress(Handle, "GetMFVideoOverlayObject");
		procV2Link = (tTVPV2LinkProc)GetProcAddress(Handle, "V2Link");
		procV2Unlink = (tTVPV2UnlinkProc)GetProcAddress(Handle, "V2Unlink");

		if(!procGetAPIVersion || !procGetVideoOverlayObject ||
			!procGetVideoLayerObject || !procGetMixingVideoOverlayObject ||
			!procGetMFVideoOverlayObject || !procV2Link || !procV2Unlink)
		...
		if(FAILED(procV2Link(TVPGetFunctionExporter())))
			TVPThrowExceptionMessage(TVPInvalidKrMovieDLL);
		Linked = true;
	}
	catch(...)
...
static tTVPVideoModule *TVPGetMovieVideoModule()
{
	if(TVPMovieVideoModule == NULL)
		TVPMovieVideoModule = new tTVPVideoModule();
...
static void TVPShutdownVideoOverlay()
{
	...
	if(TVPMovieVideoModule)
	{
		// This is the process-exit path.  Do not call FreeLibrary here: Media
		// Foundation and window-manager work queues can still hold callbacks or
		// window procedures implemented by krmovie.dll.  The OS releases the
		// module after all process threads have stopped, which is the only safe
		...
		iTJSDispatch2 *tjs_obj)
{
	...
	// The movie DLL must stay loaded until every live native overlay has
	// released its iTVPVideoOverlay.  In particular, MF may still own an
	// asynchronous callback while the script object itself is alive.
	...
	try
	{
		tTVPVideoModule *module = TVPGetMovieVideoModule();
		{
			if(Mode == vomLayer)
				...
				module->GetMixingVideoOverlayObject(EventQueue.GetOwner(), istream, name.c_str(), ext.c_str(), size, &VideoOverlay);
			else if(Mode == vomMFEVR)
				module->GetMFVideoOverlayObject(EventQueue.GetOwner(), istream, name.c_str(), ext.c_str(), size, &VideoOverlay);
			else
