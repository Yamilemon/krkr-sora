...
, BlendMode(tTVPBlendMode::bmAlpha)
,CurrentScissorRect(-1,-1,-1,-1)
...
void tTJSNI_Canvas::SetSurfaceSize(int width, int height)
{
	SurfaceWidth = width;
...
//----------------------------------------------------------------------
// method
void tTJSNI_Canvas::Capture( class tTJSNI_Bitmap* bmp, int x, int y, int w, int h ) {
...
//----------------------------------------------------------------------
void tTJSNI_Canvas::Capture( const class iTVPTextureInfoIntrface* texture, int x, int y, int w, int h ) {
	tjs_int sw = CanvasWidth;
	tjs_int sh = CanvasHeight;
...
void tTJSNI_Canvas::SetupTextureDrawing( tTJSNI_ShaderProgram* shader, const iTVPTextureInfoIntrface* tex, tTJSNI_Matrix32* mat, const tTVPPoint& vpSize ) {
	...
	GLint opacityLoc = shader->FindLocation(std::string("a_opacity"));
	if (opacityLoc >= 0) {
...
void tTJSNI_Canvas::ApplyClipRect() {
	if( ClipRectInstance ) {
		...
		const tTVPRect& rect = CurrentScissorRect;
		int sh = CanvasHeight;
		glScissor( rect.left, sh-rect.bottom, rect.get_width(), rect.get_height() );
...
//----------------------------------------------------------------------
// prop
void tTJSNI_Canvas::SetRenderTargetObject( const tTJSVariant & val ) {
...
{
	// Canvasでの一連の描画処理を呼び出す
	// OGLDrawDevice.onDraw 以外からの呼び出し処理用
	// canvas.draw(target, action, *)

	// 以下の形で一連の描画処理を呼び出す
	// actionが空文字 target(canvas, *)
	// actionが文字列 target.action(canvas, *)
	
	TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Canvas);
	if(numparams < 1) return TJS_E_BADPARAMCOUNT;

	tTJSVariantClosure ActionOwner = param[0]->AsObjectClosure();
	ttstr ActionName;
	if(numparams >= 2 && param[1]->Type() != tvtVoid)
		ActionName = *param[1]; // action function to be called

	if(ActionOwner.Object)
	{
		// 第一引数:canvas
		tTJSVariant canvas(objthis, objthis);
		std::vector<tTJSVariant *> pparams;
		pparams.push_back(&canvas);
		// 残りの引数を追加
		for (int i=2;i<numparams;i++) {
			pparams.push_back(param[i]);
		}

		_this->BeginDrawing();
		ActionOwner.FuncCall(0, 
			ActionName.IsEmpty() ? NULL :ActionName.c_str(),
			ActionName.IsEmpty() ? NULL :ActionName.GetHint(),
			result, pparams.size(), &pparams[0], NULL);
		_this->EndDrawing();
	}
	return TJS_S_OK;
}
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/capture)
{
	...
	int x = numparams > 1 ? (tjs_int)*param[1] : 0;
	int y = numparams > 2 ? (tjs_int)*param[2] : 0;
	int w = numparams > 3 ? (tjs_int)*param[3] : 0;
	int h = numparams > 4 ? (tjs_int)*param[4] : 0;

	const iTVPTextureInfoIntrface* texture = TVPGetTextureInfo( param[0] );
	if( texture ) {
		_this->Capture( texture, x, y, w, h );
	} else {
		tTJSNI_Bitmap* bmp = (tTJSNI_Bitmap*)TJSGetNativeInstance( tTJSNC_Bitmap::ClassID, param[0] );
		if( !bmp ) return TJS_E_INVALIDPARAM;
		_this->Capture( bmp, x, y, w, h );
	}
...
TJS_BEGIN_NATIVE_PROP_DECL( renderTarget )
{
	...
	TJS_BEGIN_NATIVE_PROP_SETTER
	{
		...
		if( param->Type() == tvtObject  ) {
			_this->SetRenderTargetObject( *param );
		} else {
			_this->SetRenderTargetObject( tTJSVariant() );
		}
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_SETTER
