...
STDMETHODIMP tTVPPlayerCallback::Invoke( IMFAsyncResult *pAsyncResult ) {
	tTVPMFPlayer *owner = owner_;
	owner->EnterCallback();

	HRESULT hr;
	MediaEventType met = MESessionClosed;
	CComPtr<IMFMediaEvent> pMediaEvent;
	// Keep the session alive while this callback is executing; ReleaseAll can
	// otherwise release it concurrently on the UI thread.
	CComPtr<IMFMediaSession> mediaSession = owner->GetMediaSession();
	if( mediaSession && SUCCEEDED(hr = mediaSession->EndGetEvent( pAsyncResult, &pMediaEvent )) ) {
		if( SUCCEEDED(hr = pMediaEvent->GetType(&met)) ) {
			PROPVARIANT pvValue;
			PropVariantInit(&pvValue);
			if( met == MESessionClosed ) {
				owner->OnMediaSessionClosed();
			} else if( !owner->IsShuttingDown() ) switch( met ) {
			...
			case MESessionTopologyStatus: {
				...
				owner->OnTopologyStatus(status);
				break;
				}
			}
			PropVariantClear(&pvValue);
		}
		// Close ends the event queue.  Re-registering after MESessionClosed can
		// leave this DLL-owned callback executable after krmovie.dll unloads.
		if( met != MESessionClosed )
...
tTVPMFPlayer::tTVPMFPlayer() {
	...
	RefCount = 1;
	Shutdown = false;
	SessionClosedEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
	CallbackIdleEvent = CreateEvent(NULL, TRUE, TRUE, NULL);
	CallbackCount = 0;
...
tTVPMFPlayer::~tTVPMFPlayer() {
	...
	MFShutdown();
	CoUninitialize();
	if( CallbackIdleEvent ) CloseHandle(CallbackIdleEvent);
	if( SessionClosedEvent ) CloseHandle(SessionClosedEvent);
}
...
void tTVPMFPlayer::OnMediaSessionClosed() {
	if( SessionClosedEvent ) SetEvent(SessionClosedEvent);
}

void tTVPMFPlayer::EnterCallback() {
	if( InterlockedIncrement(&CallbackCount) == 1 && CallbackIdleEvent )
		ResetEvent(CallbackIdleEvent);
}

void tTVPMFPlayer::LeaveCallback() {
	if( InterlockedDecrement(&CallbackCount) == 0 && CallbackIdleEvent )
		SetEvent(CallbackIdleEvent);
}
/*
...
{
	// WM_DESTROY and the destructor can both arrive during shutdown.  Only the
	// first caller may tear down the Media Foundation event queue.
	if( InterlockedExchange(&Shutdown, TRUE) ) return;

	if( MediaSession.p ) {
		MediaSession->Stop();
		if( SessionClosedEvent ) ResetEvent(SessionClosedEvent);
		if( SUCCEEDED(MediaSession->Close()) && SessionClosedEvent ) {
			// Close is asynchronous.  Keep the object and its callback alive until
			// Media Foundation has delivered MESessionClosed.
			WaitForSingleObject(SessionClosedEvent, 5000);
		}
		if( MediaSource.p ) {
	...
	if( CallbackIdleEvent ) {
		WaitForSingleObject(CallbackIdleEvent, 5000);
	}
	if( PlayerCallback ) {
